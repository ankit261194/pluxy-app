...
const INITIAL_STATE = {
  // Default to null so fresh installs start at Login / Registration screen
  currentUser: null,
  
  // Permanent reference for Founder & Lead Developer Ankit Chaudhary
  defaultFounder: {
    id: "user_ankit",
...
class Store {
  constructor() {
    ...
    if (saved) {
      try {
        ...
        // Bind currentUser strictly from active authenticated session
        let activeSession = null;
        try {
          const s = localStorage.getItem("pluxy_active_session");
          if (s) activeSession = JSON.parse(s);
        } catch(e) {}
        this.data.currentUser = activeSession;
  ...
  addComment(postId, text) {
    const post = this.data.posts.find(p => p.id === postId);
    const user = this.getCurrentUser() || { username: "user" };
    if (post && text.trim()) {
      const comment = {
        id: "c_" + Date.now(),
        user: user.username,
...
Store.prototype.addReel = function(videoUrl, caption, audioTrack) {
  ...
  const newReel = {
    ...
    audioTrack: audioTrack || "Original Audio - " + (user.displayName || user.username) + " 🎵",
    likes: 1,
    isLiked: true,
    commentsCount: 0
  };
