...
class ProfileModule {
  ...
  renderProfile() {
    ...
    const isAdmin = window.authModule ? window.authModule.isAdmin() : (user && user.role === 'admin');
    if (!this.container) return;

    if (!user) {
      this.container.innerHTML = `
        <div class="profile-banner-wrap" style="padding: 40px 20px; text-align: center;">
          <div class="profile-avatar-large" style="margin: 0 auto 16px auto; width: 90px; height: 90px; border-radius: 50%; background: rgba(56, 189, 248, 0.15); display: flex; align-items: center; justify-content: center; font-size: 40px; border: 2px dashed rgba(56, 189, 248, 0.4);">
            👤
          </div>
          <h2 style="font-size: 1.4rem; font-weight: 800; color: var(--text-primary); margin-bottom: 6px;">Sign In to Pluxy</h2>
          <p style="font-size: 0.9rem; color: var(--text-secondary); max-width: 320px; margin: 0 auto 20px auto;">
            Create an account or sign in to share posts, stories, reels and access custom profile features!
          </p>
          <div style="display: flex; gap: 12px; justify-content: center; flex-wrap: wrap;">
            <button class="btn-submit-primary" onclick="window.authModule.openAuth('login')" style="padding: 12px 24px; font-weight: 700;">
              <i class="ph-bold ph-sign-in"></i> Sign In
            </button>
            <button class="btn-submit-primary" onclick="window.authModule.openAuth('register')" style="padding: 12px 24px; font-weight: 700; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">
              <i class="ph-bold ph-user-plus"></i> Create Account
            </button>
          </div>
          <div style="margin-top: 28px;">
            <button class="btn-profile-action admin-action-btn" onclick="window.adminModule.openAdminPortal()" style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px;">
              <i class="ph-bold ph-shield-check"></i> Admin Portal
            </button>
          </div>
        </div>
      `;
          ...
          <div class="admin-master-content">
            <div class="admin-master-left">
              ...
              <div>
                ...
                <span>${isAdmin ? 'App settings, dynamic services, user management & audit trail' : 'Enter Admin Master PIN to customize and manage Pluxy'}</span>
              </div>
            </div>
